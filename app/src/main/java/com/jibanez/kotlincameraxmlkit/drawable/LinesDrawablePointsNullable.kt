...
class LinesDrawablePointsNullable(private val points: List<PointF?>, private val color: Int) : Drawable() {
    ...
    override fun draw(canvas: Canvas) {
        ...
        if (points.isNotEmpty()) {
            ...
            do {
                points[firstIndexFound]?.let { path.moveTo(it.x, it.y) }
                firstIndexFound++
            } while (points[firstIndexFound-1] == null && firstIndexFound < points.size)
